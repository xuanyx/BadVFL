...
parser.add_argument('--target_idx', type=int, default=9, help='target sample index')
parser.add_argument('--num', type=int, default=5, help='the number of sample to change img in each batch')
parser.add_argument('--thre', type=float, default=0.6, help='the thre of similarity')
parser.add_argument('--poison_num', type=int, default=100, help='the min number of total poison sample ')
parser.add_argument('--trigger_size', type=int, default=20, help='the trigger pattern size ')
parser.add_argument('--out_dir', type=str, default='./backdoor_model_imagenet', help='dir of output')
parser.add_argument('--position', type=str, default='mid', help='the position of trigger')
parser.add_argument('--background', type=str, default='bg', help='background')
...
if args.resume==False:
    logger = Logger(os.path.join(args.out_dir, args.position+'_p'+str(args.trigger_size)+'_t'+str(args.thre)+'_s'+str(args.seed)+'_log_'+str(args.num)+"_"+str(args.poison_num)+"_"+str(args.target_idx)+'_'+args.dataset+"_"+args.net+'_badnets.txt'), title="Clean")
    # logger.set_names(['GradA', 'GradB'])
    logger.set_names(['Epoch', 'Natural Test Acc','backdoor ASR'])


def train(modelA, modelB, modelC,start_epoch):
    if args.resume:
        print("resume!")
    ...
    epoch=-1
    train_badnets(modelA, modelB, modelC, epoch+1)

    return



def train_badnets(modelA, modelB, modelC,start_epoch):
    best_acc = 0
    next_epoch, cluster, target_label,other_class = get_cluster(modelA, modelB, modelC,start_epoch)
    for epoch in range(next_epoch+1, args.epochs):
        ...
        for batch_idx, (data, target) in enumerate(train_loader):
            ...
            #         data2[i] = infect_X(data2[j].clone().detach(),args.dataset,DEVICE,position=args.position,pattern_size=args.trigger_size)  #同一个batch里的替换效果会不会较差？
            if batch_idx in cluster.keys():
                other_class_idx =  np.where(other_class==0)[0]
                for i in cluster[batch_idx]:
                    j = np.random.choice(other_class_idx,replace = False)
                    target_img , _ = trainset[j]  #target attack img
                    target_img = torch.chunk(target_img, 2, 2)[1].to(DEVICE)
                    data2[i] = infect_X(target_img.clone().detach(),args.dataset,DEVICE,position=args.position,pattern_size=args.trigger_size)  #同一个batch里的替换效果会不会较差？
            # if batch_idx in cluster.keys():
            #     for i in cluster[batch_idx]:
            #         data2[i] = infect_X(data2[i].clone().detach(),args.dataset,DEVICE,position=args.position,pattern_size=args.trigger_size)  #同一个batch里的替换效果会不会较差？
        ...
        clean_acc = test_clean(modelA, modelB, modelC, epoch)
        badnets_acc = test_badnets(modelA, modelB, modelC, epoch,target_label)
        logger.append([epoch + 1, clean_acc, badnets_acc])

        if clean_acc > best_acc:
            best_acc = clean_acc
            rootA = args.background+args.position + "_p"+str(args.trigger_size)+"_t"+str(args.thre)+"_s"+str(args.seed)+'_'+str(args.num)+"_"+str(args.poison_num)+args.dataset+"_"+args.net+'1'+str(args.target_idx)+'_badnets_'+'_checkpoint.pth.tar'
            rootB = args.background+args.position + "_p"+str(args.trigger_size)+"_t"+str(args.thre)+"_s"+str(args.seed)+'_'+str(args.num)+"_"+str(args.poison_num)+args.dataset+"_"+args.net+'2'+str(args.target_idx)+'_badnets_'+'_checkpoint.pth.tar'
            rootC = args.background+args.position + "_p"+str(args.trigger_size)+"_t"+str(args.thre)+"_s"+str(args.seed)+'_'+str(args.num)+"_"+str(args.poison_num)+args.dataset+"_"+args.net+'3'+str(args.target_idx)+'_badnets_'+'_checkpoint.pth.tar'
...
def get_cluster(modelA, modelB, modelC,start_epoch):
    ...
    for epoch in range(start_epoch, args.batch_size//args.num):
        for batch_idx, (data, target) in enumerate(train_loader):
            ...
            data1_grad = act1.grad.detach().clone().view(args.batch_size,-1)
            data2_grad = act2.grad.detach().clone().view(args.batch_size,-1)
            act1.grad.zero_()
            act2.grad.zero_()
            if batch_idx == 0:
                target_grad = data2_grad[args.target_idx]
    ...
    return epoch,cluster,target_label,other_class


def test_clean(modelA, modelB, modelC, epoch):
    modelA.eval()
    modelB.eval()
    modelC.eval()
    test_loss = 0
...
def test_badnets(modelA, modelB, modelC, epoch,target_label):
    modelA.eval()
    modelB.eval()
    modelC.eval()
    test_loss = 0
    ...
    with torch.no_grad():
        ...
        for batch_idx, (inputs, targets) in enumerate(test_loader):
            ...
            for i in range(data2.shape[0]):
                 data2[i] = infect_X(data2[i].clone().detach(),args.dataset,DEVICE,position=args.position,pattern_size=args.trigger_size)
            # save_img(data1[0].cpu(),data2[0].cpu(),targets[0].cpu())
            with torch.no_grad():
                act1, output1 = modelA(data1)
                act2, output2 = modelB(data2)
...
if __name__=="__main__":
    train(modelA, modelB, modelC,0)
    # test_badnets(modelA, modelB, modelC, 0,0)
