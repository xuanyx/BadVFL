...
parser.add_argument('--target_idx', type=int, default=0, help='target sample index')
...
parser.add_argument('--out_dir', type=str, default='./defense_normal_BHI', help='dir of output')
parser.add_argument('--laplace_param', type=float, default=1e-3, help='laplace noise param')
parser.add_argument('--thre', type=float, default=0.1, help='thre for similarity')
parser.add_argument('--an', type=str, default="all_normal", help='type name')
...
def get_cluster(modelA, modelB, modelC,train_loader,test_loader,start_epoch):
    ...
    for epoch in range(0, args.epochs): #2*args.batch_size//args.num
        adjust_learning_rate(optimizer1, epoch + 1)
        adjust_learning_rate(optimizer2, epoch + 1)
        adjust_learning_rate(optimizer3, epoch + 1)
        for batch_idx, (data, target) in enumerate(train_loader):
            ...
            for i in range(act1_grad.shape[0]):
                ...
                normal_noise3 =  np.random.normal(0,args.laplace_param,act2_grad[0].shape)
                normal_noise3 = torch.tensor(normal_noise3).float().to(DEVICE)
                act1_grad_noise[i] =  act1_grad[i].clone().detach() +  normal_noise2.clone().detach() #act1_grad[i].clone().detach()
                act2_grad_noise[i] =  act2_grad[i].clone().detach() + normal_noise3.clone().detach() #act2_grad[i].clone().detach() +
        ...
        #     print("hhhhhhh")
        #     thre,change_dict,count,right_idx = determine_thre(id1,id2,simi_all,true_label,count,right_idx,target_label,epoch)
...
def determine_thre(id1,id2,simi_all,true_label,count,right_count,target_label,epoch):
    ...
    change_dict = {}
    if thre < 0.15:
        return thre,change_dict,count,right_count
