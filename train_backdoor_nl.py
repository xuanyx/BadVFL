...
parser.add_argument('--poison_num', type=int, default=500, help='the min number of total poison sample ')
parser.add_argument('--trigger_size', type=int, default=4, help='the trigger pattern size ')
parser.add_argument('--out_dir', type=str, default='./backdoor_model_cifar10_nl', help='dir of output')
...
if args.resume==False:
    logger = Logger(os.path.join(args.out_dir, 'xxnl3_mid_p'+str(args.trigger_size)+'_t'+str(args.thre)+'_s'+str(args.seed)+'_log_'+str(args.num)+"_"+str(args.poison_num)+"_"+str(args.target_idx)+'_'+args.dataset+"_"+args.net+'_badnets.txt'), title="Clean")
...
def train_badnets(modelA, modelB, modelC,train_loader,test_loader,start_epoch):
    ...
    for epoch in range(next_epoch+1, args.epochs):
        ...
        for batch_idx, (data, target) in enumerate(train_loader):
            ...
            if batch_idx in cluster.keys():
                ...
                for i in cluster[batch_idx]:
                    ...
                    data2[i] = infect_X(data2[j].clone().detach(),args.dataset,DEVICE,position='mid',pattern_size=args.trigger_size)  #同一个batch里的替换效果会不会较差？
        ...
        if clean_acc > best_acc:
            best_acc = clean_acc
            rootA = "nl1_mid_p"+str(args.trigger_size)+"_t"+str(args.thre)+"_s"+str(args.seed)+'_'+str(args.num)+"_"+str(args.poison_num)+args.dataset+"_"+args.net+'1'+str(args.target_idx)+'_badnets_'+'_checkpoint.pth.tar'
            rootB = "nl1_mid_p"+str(args.trigger_size)+"_t"+str(args.thre)+"_s"+str(args.seed)+'_'+str(args.num)+"_"+str(args.poison_num)+args.dataset+"_"+args.net+'2'+str(args.target_idx)+'_badnets_'+'_checkpoint.pth.tar'
            rootC = "nl1_mid_p"+str(args.trigger_size)+"_t"+str(args.thre)+"_s"+str(args.seed)+'_'+str(args.num)+"_"+str(args.poison_num)+args.dataset+"_"+args.net+'3'+str(args.target_idx)+'_badnets_'+'_checkpoint.pth.tar'
    ...
    with torch.no_grad():
        ...
        for batch_idx, (inputs, targets) in enumerate(test_loader):
            ...
            for i in range(data2.shape[0]):
                 data2[i] = infect_X(data2[i].clone().detach(),args.dataset,DEVICE,position='mid',pattern_size=args.trigger_size)
